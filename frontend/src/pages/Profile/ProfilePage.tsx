...
const ProfilePage: React.FC = () => {
  ...
  return (
    <div className={styles.profileContainer}>
      {(!user || isLoadingUserEvents) && (
        <Spinner message="Загрузка данных профиля..." />
      )}
      ...
      {user && (
        <>
          ...
          <div className={styles.myEventsHeader}>
            {' '}
            <h2 className={styles.subTitle}>Мои мероприятия</h2>{' '}
            <Link to="/event/new" className={styles.createEventButton}>
              Создать новое
            </Link>{' '}
          </div>
